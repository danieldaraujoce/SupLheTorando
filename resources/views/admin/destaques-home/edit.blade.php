...
<div class="rounded-2xl border border-gray-200 bg-white p-6">
    {{-- Ação do formulário aponta para a rota de UPDATE passando o objeto completo --}}
    <form action="{{ route('admin.destaques-home.update', $destaque) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="titulo" placeholder="Título do card de destaque"
                            value="{{ old('titulo', $destaque->titulo) }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="subtitulo" placeholder="Texto curto abaixo do título"
                            value="{{ old('subtitulo', $destaque->subtitulo) }}"
                            class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10">
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <textarea name="descricao" rows="3" placeholder="Texto principal do card"
                            ...
                            required>{{ old('descricao', $destaque->descricao) }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <img src="{{ $destaque->image_url }}" alt="Imagem Atual" class="h-16 w-20 object-cover rounded-md">
                         ...
                         <p class="text-xs text-gray-500 mt-2">Envie uma nova imagem apenas se desejar substituí-la.</p>
                    </div>
                    <div>
                        <label class="mb-3 block text-sm font-medium text-black">Valor em Moedas (Opcional)</label>
                        <input type="number" name="valor_moedas" placeholder="Ex: 100" value="{{ old('valor_moedas', $destaque->valor_moedas) }}"
                            class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10">
                    </div>
                    <div>
                        <label class="mb-3 block text-sm font-medium text-black">Ordem de Exibição</label>
                        <input type="number" name="ordem" value="{{ old('ordem', $destaque->ordem) }}"
                            class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10">
                    </div>
                     <div>
                        ...
                        <select name="status" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10" required>
                            <option value="ativo" {{ old('status', $destaque->status) == 'ativo' ? 'selected' : '' }}>Ativo (Visível no site)</option>
                            <option value="inativo" {{ old('status', $destaque->status) == 'inativo' ? 'selected' : '' }}>Inativo (Oculto no site)</option>
                        </select>
                    </div>
                    <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="texto_botao" placeholder="Ex: Participe Agora"
                                value="{{ old('texto_botao', $destaque->texto_botao) }}"
                                class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10"
                                required>
                        </div>
                        <div>
                            ...
                            <input type="text" name="link_botao" placeholder="Ex: /register ou https://..."
                                value="{{ old('link_botao', $destaque->link_botao) }}"
                                class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10"
                                required>
                        </div>
                    </div>
