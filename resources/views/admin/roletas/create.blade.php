...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <h2 class="text-2xl font-bold text-black">Criar Nova Roleta</h2>
    <nav>
...
<div class="rounded-2xl border border-gray-200 bg-white p-6">
    <form action="{{ route('admin.roletas.store') }}" method="POST">
        @csrf
        <h4 class="mb-6 text-lg font-semibold text-gray-800">Informações da Roleta</h4>

        <div class="flex flex-col gap-6">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    <label class="mb-3 block text-sm font-medium text-black">Título da Roleta</label>
                    <input type="text" name="titulo" placeholder="Ex: Roleta de Verão" class="h-11 w-full rounded-lg border border-gray-300 px-4" required>
                </div>
                <div>
                    <label class="mb-3 block text-sm font-medium text-black">Descrição</label>
                    <textarea name="descricao" rows="1" placeholder="Finalidade da roleta" class="w-full rounded-lg border border-gray-300 py-2.5 px-4"></textarea>
                </div>
                <div>
        ...
        <div class="mt-9 flex justify-end gap-4 border-t border-gray-200 pt-6">
            <a href="{{ route('admin.roletas.index') }}" class="flex justify-center rounded-lg border border-gray-300 bg-white px-5 py-2.5">Cancelar</a>
            <button type="submit" class="flex justify-center rounded-lg bg-primary px-5 py-2.5 font-medium text-white">Salvar e Adicionar Fatias</button>
        </div>
    </form>
</div>
@endsection