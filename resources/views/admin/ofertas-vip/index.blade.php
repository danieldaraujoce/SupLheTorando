...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <h2 class="text-2xl font-bold text-black">Ofertas VIP</h2>
    <nav>
        <ol class="flex items-center gap-1.5">
            <li><a class="text-sm text-gray-500" href="{{ route('admin.dashboard') }}">Dashboard /</a></li>
            <li class="text-sm font-medium text-primary">Ofertas VIP</li>
        </ol>
    </nav>
</div>
...
<div class="rounded-2xl border border-gray-200 bg-white p-6">
    ...
    <div
        ...
        <div>
            <h4 class="text-xl font-semibold text-black">Lista de Ofertas VIP</h4>
            <p class="text-sm text-gray-500 mt-1">Gerencie as ofertas exclusivas para clientes VIP.</p>
        </div>
        <a href="{{ route('admin.ofertas-vip.create') }}"
        ...
        <table class="w-full table-auto">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-50">
                    <td class="py-3 px-6"><img src="{{ Storage::url($oferta->imagem_capa) }}"
                            alt="{{ $oferta->titulo }}" class="h-12 w-12 object-cover rounded-md"></td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center justify-center space-x-3">
                            <a href="{{ route('admin.ofertas-vip.edit', $oferta->id) }}"
                                class="rounded-lg bg-primary py-2 px-4 text-sm font-medium text-white hover:bg-opacity-90">Editar</a>
                            <form action="{{ route('admin.ofertas-vip.destroy', $oferta->id) }}" method="POST"
                ...
                @empty
                <tr>
                    <td colspan="5" class="py-10 px-4 text-center text-gray-500">Nenhuma oferta VIP encontrada.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
